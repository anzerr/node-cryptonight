...
let lr = lineReader.createInterface({
	input: fs.createReadStream('./test/tests/data/cryptonight_light-1.txt')
});
lr.on('line', (line) => {
	let line_data = line.split(/ (.+)/);
	line_count += 1;
	multiHashing.cryptonight_light_async(Buffer.from(line_data[1], 'hex'), 1, (err, result) => {
...
lr.on('error', (err) => console.log('cryptonight_light_async', err));
