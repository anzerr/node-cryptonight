...
let lr = lineReader.createInterface({
	input: fs.createReadStream('./test/tests/data/cryptonight.txt')
});
lr.on('line', (line) => {
	let line_data = line.split(/ (.+)/);
	line_count += 1;
	multiHashing.cryptonight_async(Buffer.from(line_data[1]), (err, result) => {
...
lr.on('error', (err) => console.log('compare_both', err));
