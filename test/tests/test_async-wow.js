...
let lr = lineReader.createInterface({
	input: fs.createReadStream('./test/tests/data/cryptonight-wow.txt')
});
lr.on('line', (line) => {
	let line_data = line.split(' ');
	line_count += 1;
	multiHashing.cryptonight_async(Buffer.from(line_data[1], 'hex'), 12, parseInt(line_data[2]), (err, result) => {
...
lr.on('error', (err) => console.log('cryptonight-wow_async', err));
